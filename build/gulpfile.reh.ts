...
['reh', 'reh-web'].forEach(type => {
	...
	// Registered so the server build can be driven one phase per process. The
	// all-in-one `vscode-reh-*` task holds the compile, bundle and package stages
	// in a single Node heap, which does not fit on a small machine.
	task.task(bundleTask);
