---
name: ux-css-layout
description: BatikCode CSS conventions, file organization, class naming, standard sizes, SplitView/Grid layout, scrollable content, responsive layout, and text overflow/ellipsis patterns. Use when writing CSS, building layouts, or fixing text truncation issues.
---
...
BatikCode does **not** use CSS media queries. Instead, it uses a **programmatic constraint-based layout system**:
...
This pattern is used throughout BatikCode — for example, `.monaco-icon-label-container` sets `min-width: 0` and `flex: 1` to allow label text to truncate.
...
BatikCode ships a design-system **size** ramp, registered in
