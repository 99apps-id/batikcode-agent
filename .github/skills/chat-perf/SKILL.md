---
name: chat-perf
description: Run chat perf benchmarks and memory leak checks against the local dev build or any published BatikCode version. Use when investigating chat rendering regressions, validating perf-sensitive changes to chat UI, or checking for memory leaks in the chat response pipeline.
---
...
- To compare performance between two BatikCode releases
- In CI to gate PRs that touch chat UI code

## Quick start

```bash
# Run perf regression test (compares local dev build vs BatikCode 1.115.0):
...
Launches BatikCode via Playwright Electron, opens the chat panel, sends a message with a mock LLM response, and measures timing, layout, and rendering metrics. By default, downloads BatikCode 1.115.0 as a baseline, benchmarks it, then benchmarks the local dev build and compares.
...
| `--setting <k=v>` | — | Set a BatikCode setting override for all builds (repeatable). |
| `--test-setting <k=v>` | — | Set a BatikCode setting override for the test build only. |
| `--baseline-setting <k=v>` | — | Set a BatikCode setting override for the baseline build only. |
...
Both `--build` and `--baseline-build` accept local paths to BatikCode executables. This enables apples-to-apples comparisons between any two builds:
...
Use `--setting`, `--test-setting`, and `--baseline-setting` to inject BatikCode settings into the launched instance. This is useful for A/B testing experimental features:
...
Launches one BatikCode session, sends N messages sequentially, forces GC between each, and measures renderer heap and DOM node count. Uses **linear regression** on the samples to compute per-message growth rate, which is compared against a threshold.
...
| `--setting <k=v>` | — | Set a BatikCode setting override (repeatable). |
...
- **auto-perf-optimize** — For launching BatikCode, driving a scenario, and capturing heap snapshots or CPU profiles automatically before doing low-level analysis.
