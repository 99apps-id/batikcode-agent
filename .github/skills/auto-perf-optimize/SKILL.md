...
description: "Run agent-driven BatikCode performance or memory investigations. Use when asked to launch Code OSS, automate a BatikCode scenario, run the Chat memory smoke runner, capture renderer heap snapshots, take workflow screenshots, compare run summaries, or drive a repeatable scenario before heap-snapshot analysis."
metadata:
  allowed-tools: Bash(npx @playwright/cli:*)
---

# BatikCode Performance Workflow

Drive a repeatable BatikCode scenario, collect memory/performance artifacts, verify that the scenario actually happened, then hand the resulting heap snapshots to the generic heap-snapshot-analysis skill when object-level investigation is needed.

## When to Use

- User describes a BatikCode workflow and asks whether it leaks or grows memory
- User asks the agent to launch BatikCode, drive a scenario, and capture heap snapshots
...
- User wants a new automation runner for a non-Chat BatikCode scenario
