...
description: Use when running integration tests in the BatikCode repo. Covers scripts/test-integration.sh (macOS/Linux) and scripts/test-integration.bat (Windows), their supported arguments for filtering, and the difference between node.js integration tests and extension host tests.
---

# Running Integration Tests

Integration tests in BatikCode are split into two categories:
...
2. **Extension host tests** - tests embedded in built-in extensions under `extensions/` (API tests, Git tests, TypeScript tests, etc.). These launch a full BatikCode instance with `--extensionDevelopmentPath`.
...
Tests run against compiled JavaScript output. Ensure the `BatikCode - Build` watch task is running or that compilation has completed before running tests.
