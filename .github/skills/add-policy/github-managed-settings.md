...
Copilot configuration (delivered to BatikCode via native MDM or the GitHub server) flows
into BatikCode's policy stack and locks a setting. It is a companion to `SKILL.md` — read
...
## The big idea: one canonical bag, three delivery channels (in BatikCode)
...
BatikCode implements **three** channels feeding the bag, matching the external schema's
...
All three BatikCode channels converge in `AccountPolicyService.getPolicyData()`.
...
the BatikCode bag is **flattened** to dot-paths — e.g. the schema's nested
...
> API source of truth, and keep the BatikCode `managedSettings` declarations aligned with
> what the server actually projects into the bag):
> - `extraKnownMarketplaces`: the schema permits source kinds `github` / `git` /
>   `directory`, but the BatikCode normalizer only accepts `github` and `git` —
...
enterprise/org/user. Inside BatikCode the bag has already been collapsed to a single
