...
| `copilot_chat.*` | Original BatikCode-only namespace. Several keys remain for backwards compatibility. | **Legacy — keep emitting; do not add new keys here.** |
...
3. `agent_monitoring.md` ("BatikCode Settings", "Environment Variables", "Activation", "Commands" tables) **and** `agent_monitoring_arch.md` ("Activation Channels", "Agent-Specific Env Var Translation" tables).
...
- The Agent Debug Log panel in BatikCode still shows the full span tree for foreground, Copilot CLI, and Claude sessions.
