---
name: unit-tests
description: Use when running unit tests in the BatikCode repo. Covers the runTests tool, scripts/test.sh (macOS/Linux) and scripts/test.bat (Windows), and their supported arguments for filtering, globbing, and debugging tests.
---
...
Tests run against compiled JavaScript output. Ensure the `BatikCode - Build` watch task is running or that compilation has completed before running tests. Test failures caused by stale output are a common pitfall.
