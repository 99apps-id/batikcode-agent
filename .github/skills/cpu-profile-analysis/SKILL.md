...
- Analyzing BatikCode user timing marks like `code/didResolveTextFileEditorModel` (trace files)
- Understanding multi-process behavior (Browser, Renderer, GPU processes in trace files)

## Detecting File Type

- **`.cpuprofile`**: Top-level JSON with `nodes`, `samples`, `timeDeltas` keys. Created by the BatikCode profiler.
...
- `$vscode`: Optional BatikCode metadata
...
DevTools traces are the future of perf tracing for BatikCode. They are created from the built-in Electron/Chrome DevTools Performance tab and contain far more information than `.cpuprofile` files.
...
| `blink.user_timing` | BatikCode performance marks (e.g. `code/willResolveTextFileEditorModel`, `code/didResolveTextFileEditorModel`) |
...
|---------|------|------------|
| **Renderer** (pid varies) | BatikCode's renderer process -- where JS runs | `CrRendererMain` (main thread) |
...
For BatikCode perf analysis, focus on the **Renderer process, CrRendererMain thread** -- this is where JavaScript execution, layout, and painting happen.
...
BatikCode emits `performance.mark()` calls that appear as `blink.user_timing` events. These are the most direct way to measure BatikCode-specific milestones:
...
- **User timing marks**: BatikCode milestone events and their timestamps
...
- Use `args.data.url` in `FunctionCall` and `EvaluateScript` events to map back to BatikCode source files (paths like `vscode-file://vscode-app/Users/.../out/vs/...`).
