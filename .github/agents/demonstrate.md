---
name: Demonstrate
description: Agent for demonstrating BatikCode features
...
5. Start BatikCode automation using `vscode_automation_start`
...
## Pointers for Controlling BatikCode

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for BatikCode interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout BatikCode) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
