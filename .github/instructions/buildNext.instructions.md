...
**The `BatikCode - Build` task is NOT needed to validate changes in the `build/` folder!**

Build scripts in `build/` are TypeScript files that run directly with `node` (e.g., `node build/next/index.ts`). They are not compiled by the main BatikCode build.
...
The default `BatikCode - Build` task now runs three parallel watchers:
...
- **`build/next/index.ts`**: Watch mode emits `Starting transpilation...` / `Finished transpilation with N errors after X ms` for BatikCode problem matcher.
