...
suite('getFlows', () => {
	...
	const testCases: Array<{ label: string; query: IFlowQuery; expectedFlows: Flows[] }> = [
		{
			label: 'BatikCode Desktop. Local filesystem. GitHub.com',
			query: {
		...
		{
			label: 'BatikCode Desktop. Local filesystem. GitHub Hosted Enterprise',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: true,
		...
		{
			label: 'BatikCode Desktop. Local filesystem. GitHub Enterprise Server',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: true,
	...
	suite('preferDeviceCodeFlow configuration', () => {
		...
		test('returns device code flow first when preferDeviceCodeFlow is true - BatikCode Desktop', async () => {
			const config = vscode.workspace.getConfiguration('github-authentication');
