...
let cachedConfig: SSHConfiguration | undefined;
...
export default class SSHConfiguration {
    ...
    static async loadFromFS(): Promise<SSHConfiguration> {
        const currentPath = getSSHConfigPath();
        if (cachedConfig && cachedConfigPath === currentPath) {
            return cachedConfig;
        }
        const config = await parseSSHConfigFromFile(currentPath, true);
        config.push(...await parseSSHConfigFromFile(systemSSHConfig, false));

        cachedConfigPath = currentPath;
        cachedConfig = new SSHConfiguration(config);
        return cachedConfig;
    }
