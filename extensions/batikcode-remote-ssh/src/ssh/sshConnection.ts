...
export interface SSHConnectConfig extends ConnectConfig {
    ...
    /** TCP socket connect timeout in milliseconds. Defaults to OS default (typically ~21s on Windows). */
...
export default class SSHConnection extends EventEmitter {
    ...
    connect(c?: SSHConnectConfig): Promise<SSHConnection> {
        ...
        this.__$connectPromise = new Promise(async (resolve, reject) => {
            ...
            // For direct connections (no proxy/sock), create a TCP socket with a
            // configurable connect timeout. The ssh2 library's readyTimeout only
            // covers the SSH handshake, not the TCP socket connection. Without
            // this, the OS-level TCP timeout (~21s on Windows) applies.
            const connectConfig = { ...this.config };
            let preConnectedSock: net.Socket | undefined;
            if (connectConfig.host && !connectConfig.sock) {
                preConnectedSock = new net.Socket();
                const socketTimeout = connectConfig.socketConnectTimeout ?? 30_000;
                    ...
                    await new Promise<void>((socketResolve, socketReject) => {
                        const timer = setTimeout(() => {
                            socketError = new Error(
                                `TCP connection to ${connectConfig.host}:${connectConfig.port} timed out after ${Math.round(socketTimeout / 1000)}s. ` +
                                'Check that the host is reachable and port 22 is not blocked by a firewall.'
                            );
                            preConnectedSock!.destroy();
                            socketReject(socketError);
                        }, socketTimeout);

                        preConnectedSock!.once('connect', () => {
                            clearTimeout(timer);
                            socketResolve();
                        });

                        preConnectedSock!.once('error', (err: Error) => {
                            clearTimeout(timer);
                            socketError = err;
                            socketReject(err);
                        });

                        preConnectedSock!.connect(connectConfig.port!, connectConfig.host!);
                    });

                    // Socket is connected; pass it to ssh2 for the SSH handshake.
                    delete connectConfig.host;
                    connectConfig.sock = preConnectedSock;
                } catch (err) {
                    preConnectedSock!.destroy();
                    this.__$connectPromise = null;
                    return reject(socketError || err);
                }
            }
