...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('batikcoderemotessh.openEmptyWindow', (host?: string | { hostName: string }) => promptOpenRemoteSSHWindow(false, typeof host === 'object' ? host.hostName : host)));
    context.subscriptions.push(vscode.commands.registerCommand('batikcoderemotessh.openEmptyWindowInCurrentWindow', (host?: string | { hostName: string }) => promptOpenRemoteSSHWindow(true, typeof host === 'object' ? host.hostName : host)));
    context.subscriptions.push(vscode.commands.registerCommand('batikcoderemotessh.openConfigFile', () => openSSHConfigFile()));
    context.subscriptions.push(vscode.commands.registerCommand('batikcoderemotessh.showLog', () => logger.show()));
}
