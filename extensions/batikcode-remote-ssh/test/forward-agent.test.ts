...
const USERNAME = 'batikcoderemotessh';
const PASSWORD = 'batikcoderemotessh';

const containerName = `batikcode-remote-ssh-test-${randomUUID()}`;
...
it('forwards the agent through a socket that stays alive', async () => {
  vol.fromJSON({
    ...
    '/data/vscodium/extensions/batikcode-remote-ssh/src/scripts/server-setup.sh': SERVER_SETUP,
  });
