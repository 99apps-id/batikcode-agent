...
- Open `/extensions/css-language-features/` as the workspace in BatikCode
...
- Run the [`Launch Extension`](https://github.com/microsoft/vscode/blob/master/extensions/css-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new BatikCode instance with the `css-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  BatikCode window opened on `css-language-features`.
...
This extension wraps the css language service into a Language Server for BatikCode.
