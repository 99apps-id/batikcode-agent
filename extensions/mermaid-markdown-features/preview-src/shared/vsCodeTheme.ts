...
/**
 * Identifier for the custom Mermaid theme that is derived from the current BatikCode color theme.
 */
export const vsCodeMermaidTheme = 'vscode';

/**
 * Returns `true` when the active BatikCode theme is a dark (or non-light high contrast) theme.
 */
...
/**
 * The Mermaid `themeVariables` derived from the active BatikCode color theme, plus a string
...
/**
 * Resolves and caches Mermaid `themeVariables` derived from the active BatikCode color theme.
 *
...
export class VsCodeMermaidThemeTracker {
	...
	/**
	 * The Mermaid theme variables derived from the active BatikCode theme. Computed lazily on
	 ...
	 * picking the dark or light slot based on the active BatikCode theme and translating the
	 * `'vscode'` sentinel into mermaid's `base` theme plus BatikCode-derived variables.
	 */
...
function computeVsCodeMermaidThemeVariables(): VsCodeMermaidThemeVariables {
	...
	// as the accent palette for various diagram types. Map to the BatikCode chart accent colors.
	const chartPalette = [
		'--vscode-charts-blue',
		'--vscode-charts-green',
