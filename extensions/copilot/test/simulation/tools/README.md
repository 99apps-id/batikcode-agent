...
This folder contains the tool call simulation test (s-test) harness for running and debugging tool invocations in the BatikCode Copilot repo.
...
You can run the tool call s-test from the terminal or from BatikCode using launch configurations.

### 1. From BatikCode: Debugging

You can debug the tool call s-test using the built-in launch configuration:

1. Open the Run & Debug panel in BatikCode (`Ctrl+Shift+D`).
