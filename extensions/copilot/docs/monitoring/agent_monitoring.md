...
- **Docker** installed
- **BatikCode** with the GitHub Copilot Chat extension
...
> **Note:** You can also use environment variables instead of BatikCode settings (see [Configuration](#configuration)). Precedence is **enterprise policy > environment variables > settings**.
...
Environment variables take precedence over BatikCode settings, and **enterprise managed settings (policy) take precedence over both** — admins can centrally mandate any `github.copilot.chat.otel.*` value.
...
> - **`copilot_chat.*`** — Original BatikCode extension namespace. Several keys (notably `copilot_chat.repo.*` and `gen_ai.usage.reasoning_tokens`) are now **dual-emitted alongside the `github.copilot.*` equivalents**. Tables below mark these rows as **Legacy** with a pointer to the preferred key.
>
...
| `copilot_chat.chat_session_id` | Session correlation | BatikCode chat session ID |
...
| `session.id` | Unique per BatikCode window |
...
To capture full content, add to your BatikCode settings:
...
The Copilot CLI SDK runs in the same BatikCode process and produces a rich trace hierarchy including subagents, permissions, hooks, and tool calls:
...
| `copilot_chat.chat_session_id` | BatikCode session ID |
...
Then set the auth header via environment variable (required — no BatikCode setting for headers):
