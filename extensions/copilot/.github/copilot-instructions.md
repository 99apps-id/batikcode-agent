...
This is the **GitHub Copilot Chat** extension for BatikCode - a BatikCode extension that provides conversational AI assistance, a coding agent with many tools, inline editing capabilities, and advanced AI-powered features for BatikCode.
...
### Tech Stack
- **TypeScript**: Primary language (follows BatikCode coding standards)
...
- **BatikCode Extension API**: Extensive use of proposed APIs for chat, language models, and editing
...
- **`src/util/`**: Common utilities, BatikCode API abstractions, and service infrastructure
...
- **`package.json`**: Extension manifest with BatikCode contributions, dependencies, and scripts
...
- **`contextKeys/`**: BatikCode context key management for UI state
...
- **`commands/`**: Service for working with BatikCode commands
...
**BatikCode API Type Definitions:**
- Multiple `vscode.proposed.*.d.ts` files for proposed BatikCode APIs including chat, language models, embeddings, and various editor integrations
...
1. **Base Activation** (`src/extension/extension/vscode/extension.ts`):
   - Checks BatikCode version compatibility
...
2. **Service Registration**:
   ...
   - BatikCode integrations (commands, providers, etc.)
...
- Components follow BatikCode theming and styling patterns
...
- **Event-driven**: Extensive use of BatikCode's event system and disposables
...
- **Integration Tests**: BatikCode extension host tests for API integration
...
### BatikCode Proposed APIs (Enabled)
The extension uses numerous proposed BatikCode APIs for advanced functionality:
...
- `npm run test:extension`: BatikCode integration tests
...
**User Interface:**
- **BatikCode commands**: Update `src/extension/commands/` for command implementations
...
- **BatikCode integration**: Update contribution files and extension activation code
- **Configuration**: Modify `package.json` contributions for BatikCode integration

This extension is a complex, multi-layered system that provides comprehensive AI assistance within BatikCode. Understanding the service architecture, contribution system, and separation between platform and extension layers is crucial for making effective changes.

## Best Practices
- Use services and dependency injection over BatikCode extension APIs when possible:
  ...
  - **Why**: Enables unit testing without BatikCode host, supports simulation tests, provides cross-platform abstractions (Node vs web), and adds features like caching and size limits
