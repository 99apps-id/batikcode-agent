...
export interface OTelConfigInput {
	...
	/** Service name from BatikCode setting (`github.copilot.chat.otel.serviceName`). */
	settingServiceName?: string;
	/** Enterprise-managed `service.name` (raw `telemetry.serviceName`). */
	policyServiceName?: string;
	/** Resource attributes from BatikCode setting (`github.copilot.chat.otel.resourceAttributes`). */
	...
	/** OTLP headers from BatikCode setting (`github.copilot.chat.otel.headers`). */
