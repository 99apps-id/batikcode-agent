...
describe('resolveOTelConfig', () => {
	...
	it('enables via BatikCode setting', () => {
		const config = resolveOTelConfig(makeInput({
			settingEnabled: true,
		}));
		expect(config.enabled).toBe(true);
	...
	it('env COPILOT_OTEL_ENABLED overrides BatikCode setting', () => {
		const config = resolveOTelConfig(makeInput({
	...
	it('uses BatikCode setting for exporter type', () => {
		const config = resolveOTelConfig(makeInput({
			settingEnabled: true,
	...
	it('captureContent env overrides BatikCode setting', () => {
		const config = resolveOTelConfig(makeInput({
			env: {
				'COPILOT_OTEL_ENABLED': 'true',
	...
	describe('enabledVia', () => {
		...
		it('returns setting when enabled via BatikCode setting', () => {
			const config = resolveOTelConfig(makeInput({
				settingEnabled: true,
			}));
	...
	describe('maxAttributeSizeChars', () => {
		...
		it('uses BatikCode setting when env var is unset', () => {
			const config = resolveOTelConfig(makeInput({
				settingEnabled: true,
		...
		it('env var overrides BatikCode setting', () => {
			const config = resolveOTelConfig(makeInput({
				env: {
					'COPILOT_OTEL_ENABLED': 'true',
