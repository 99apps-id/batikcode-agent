		...
		const updateRegistration = () => {
			...
			if (!disabled) {
				...
				if (!context.state.installed && !otherAgentsAndToolsDisposable.value) {
					...
					// VSCode Agent
					disposables.add(SetupChatAgent.registerOtherAgents(this.instantiationService, 'setup.vscode', 'vscode', false, localize2('vscodeAgentDescription', "Ask questions about BatikCode").value, ChatAgentLocation.Panel, undefined, context, controller).disposable);
					...
					disposables.add(SetupTool.registerTools(this.instantiationService, {
						...
						modelDescription: localize('setupToolsDescription', "Scaffold a new workspace in BatikCode"),
						userDescription: localize('setupToolsDescription', "Scaffold a new workspace in BatikCode"),
