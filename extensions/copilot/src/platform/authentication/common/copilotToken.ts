...
/**
 * A function used to determine if the org list contains a BatikCode organization
 * @param orgList The list of organizations the user is a member of
 * Whether or not it contains a BatikCode org
 */
export function containsVSCodeOrg(orgList: string[]): boolean {
	const VSCODE_ORGANIZATIONS = ['551cca60ce19654d894e786220822482'];
	// Check if the user is part of a BatikCode organization.
	for (const org of orgList) {
...
export type TokenErrorReason =
	...
	'ParseFailed' |
	/** User not authenticated with GitHub through BatikCode. Only returned from BatikCode integration layer, not from platform token minting. */
	'GitHubLoginFailed' |
