...
export class VSCodeCopilotTokenManager extends BaseCopilotTokenManager {
	...
	private async _auth(): Promise<TokenInfoOrError> {
		...
		if (session) {
			...
			if (tokenResult.kind === 'success') {
				...
				this._logService.info(`Copilot Chat: ${this._envService.getVersion()}, BatikCode: ${this._envService.vscodeVersion}`);
			}
			return tokenResult;
		} else {
	...
	private async _authShowWarnings(): Promise<ExtendedTokenInfo> {
		...
		if (tokenResult.kind === 'failure' && tokenResult.reason === 'HTTP401') {
			const message =
				'Your GitHub token is invalid. Please sign out from your GitHub account using the BatikCode accounts menu and try again.';
			if (!shown401Message) {
