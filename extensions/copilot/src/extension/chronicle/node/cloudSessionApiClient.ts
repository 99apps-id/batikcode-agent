...
export class CloudSessionApiClient {
	...
	/**
	 * List BatikCode cloud sessions for the authenticated user.
	 * Paginates through all pages and filters to only BatikCode Chat sessions.
	 */
	async listSessions(): Promise<Array<{ id: string; task_id?: string; agent_task_id?: string; agent_id?: number; state: string; created_at: string }>> {
		...
		try {
			while (true) {
				...
				// Filter to BatikCode Chat sessions only
