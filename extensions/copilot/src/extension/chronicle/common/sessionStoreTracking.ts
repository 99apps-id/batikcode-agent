...
const FILE_TRACKING_TOOLS = new Set([
	// BatikCode model-facing tool names (from ToolName enum)
 ...
 * Handles both CLI-style (edit/create with `path`) and BatikCode-style tools
...
export function extractFilePath(toolName: string, toolArgs: unknown): string | undefined {
	...
	// BatikCode tools use 'filePath', CLI tools use 'path', list_dir uses 'path',
 ...
 * Matches both BatikCode-style `mcp_github_*` and CLI-style `github-mcp-server-*` prefixes.
 */
