...
export async function baseActivate(configuration: IExtensionActivationConfiguration) {
	...
	// Check if the extension is running in a pre-release version of BatikCode
	...
	if (context.extension.packageJSON.isPreRelease && isStableVsCode) {
		// Prevent activation of the extension if the user is using a pre-release version in stable BatikCode
