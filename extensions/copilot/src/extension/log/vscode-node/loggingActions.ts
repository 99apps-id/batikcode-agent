...
- BatikCode: ${vscode.version} (${product.commit || 'out-of-source'})
...
`);
			...
			if (currentFetcher.fetcher.getUserAgentLibrary() !== activeFetcher) {
				notes.push(`- The active fetcher differs from the configured fetcher (${currentFetcher.name}). This can happen after a recent configuration change or an automatic fallback; reload the window or restart BatikCode to apply the configured fetcher.`);
			}
