...
/**
 * Convert a JSONL debug log entry into a BatikCode debug panel event.
 ...
 *   entries because BatikCode core is already displaying them for live sessions.
...
export function debugLogEntryToDebugEvent(entry: IDebugLogEntry, skipCoreEvents = true): vscode.ChatDebugEvent | undefined {
	switch (entry.type) {
		...
		case 'discovery':
			...
			// by BatikCode core via onDidReceiveChatDebugEvent for live sessions.
		...
		case 'generic':
			...
			// by BatikCode core directly for live sessions.
 ...
 * spanId collisions from OTel counter resets across BatikCode restarts.
 */
