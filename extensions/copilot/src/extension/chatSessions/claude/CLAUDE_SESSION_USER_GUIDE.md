...
This guide covers the **Claude** session target in BatikCode Copilot Chat: what it is, how to use it, and every feature available to you.

> **Quick summary:** The Claude session target lets you delegate agentic coding tasks to Anthropic's Claude Agent SDK running locally on your machine — directly inside BatikCode's chat UI. It can read your code, run shell commands, edit files, search the web, and iterate autonomously until a task is complete.
...
- **BatikCode Insiders** (the extension uses proposed APIs not available in Stable)
...
- **Signed into GitHub** — you must be authenticated in BatikCode. If you see "Not Logged In" in the status bar, sign in via the Accounts button in the sidebar before using Claude
...
1. Open BatikCode Settings (`Cmd+,` / `Ctrl+,`)
2. Search for `claudeAgent.enabled`
3. Ensure **"Enable Claude Agent sessions in BatikCode"** is checked
...
Claude sessions are persisted to disk and can be resumed across BatikCode restarts. The **Sessions view** is your central hub for browsing, searching, and resuming past sessions.
...
| **Open in New Window** | — | Open the session in a new BatikCode window |
...
Session files are stored on disk at `~/.claude/projects/<workspace-slug>/` as JSONL files, so they persist across BatikCode restarts and even across machines if the files are synced.
...
- **User settings** — BatikCode user settings
...
> **Note:** An additional `/terminal` command exists in the codebase but is currently disabled pending review. When enabled, it will create a BatikCode terminal with the Claude CLI configured to use your Copilot subscription.
...
| Tool | Description |
|------|-------------|
| **getDiagnostics** | Get language diagnostics (errors, warnings) from BatikCode's language servers |
...
Hooks let you run custom scripts at key moments in Claude's execution. They're configured via the [`/hooks`](#hooks--configure-lifecycle-hooks) slash command and stored in BatikCode settings.
...
|---------|------|---------|-------------|
| `github.copilot.chat.claudeAgent.enabled` | boolean | `true` | Enable Claude Agent sessions in BatikCode |
...
BatikCode Copilot Chat offers four session targets. Here's how Claude compares:

| Feature | Claude | Local (Copilot) | Copilot CLI | Cloud |
|---------|--------|-----------------|-------------|-------|
| **Where it runs** | Locally via Agent SDK | Locally via BatikCode | Locally in a separate Git worktree (isolated from your working copy) | Cloud (GitHub) |
...
10. **Resume sessions** — don't worry about closing BatikCode. Your Claude sessions persist and can be browsed in the [Sessions view](#sessions-view). Rename important sessions so you can find them later.
