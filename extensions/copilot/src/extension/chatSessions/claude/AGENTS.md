...
This folder contains the Claude Code integration for BatikCode Chat. It enables users to open a new Chat window and interact with a Claude Code instance directly within BatikCode. **BatikCode provides the UI, Claude Code provides the smarts.**
...
The Claude Code integration allows BatikCode's chat interface to communicate with Claude Code, Anthropic's agentic coding assistant. When a user sends a message in a BatikCode Chat window using this integration, the message is routed to a Claude Code session that can:
...
All interactions are displayed through BatikCode's native chat UI, providing a seamless experience.
...
**ClaudeAgentManager**
- Entry point for handling chat requests from BatikCode
...
- Resolves prompts by replacing BatikCode references (files, locations) with actual paths

**ClaudeCodeSession**
- Represents a single Claude Code conversation session
- Manages a queue of incoming requests from BatikCode Chat
...
- Handles tool confirmation dialogs via BatikCode's chat API
...
Formats tool invocations for display in BatikCode's chat UI:
...
Converts a persisted `IClaudeCodeSession` into BatikCode `ChatResponsePart[]` for replay in the chat UI:
...
1. **User sends message** in BatikCode Chat
...
4. **Claude Code SDK** returns streaming messages:
   ...
   - Tool use requests → shown as progress, then confirmed via BatikCode's confirmation API
...
BatikCode calls `getChatSessionInputState` to get a `ChatSessionInputState` object whose `.groups` array drives the UI. Rather than computing groups once and returning them, the pipeline keeps `groups` live: shared observables push changes into each state object whenever relevant configuration changes.
...
When BatikCode discards a `ChatSessionInputState`, the `onDidDispose` event fires and deterministically cleans up all autoruns for that state. The `onDidDispose` subscription is itself registered on the pipeline store, so it is cleaned up as part of disposal.
...
- **`context.previousInputState` path** — BatikCode already has a state for this session and is asking for a fresh one; seed from the old groups.
...
- Commands can optionally register with BatikCode Command Palette
...
interface IClaudeSlashCommandHandler {
	...
	readonly commandId?: string;          // Optional BatikCode command ID
...
- All other tools show a confirmation dialog via BatikCode's chat API
...
- `vscode-node/mcpServers/index.ts` → BatikCode-specific contributors (imports node first)
...
The integration respects BatikCode settings:
