...
export interface IClaudeSlashCommandService {
	 ...
	 *
	 * Checks `request.command` first (BatikCode slash command), then falls back to
...
export class ClaudeSlashCommandService extends Disposable implements IClaudeSlashCommandService {
	...
	constructor(
		...
		super();
		// Initialize eagerly to register BatikCode commands at startup
	...
	async tryHandleCommand(
		...
		// 1. Check request.command (BatikCode slash command selected via UI)
		if (request.command) {
	...
	private _ensureInitialized(): void {
		...
		for (const ctor of ctors) {
			...
			// Register BatikCode command if commandId is provided
