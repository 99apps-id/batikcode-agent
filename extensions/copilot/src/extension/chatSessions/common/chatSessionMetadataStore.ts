...
export interface RequestDetails {
	/** BatikCode request ID — always available, serves as primary key. */
	readonly vscodeRequestId: string;
	/** Copilot SDK request ID — may not be available until the request completes. */
	copilotRequestId?: string;
	/**
	 * Map of tool call id to BatikCode edit id, used to correlate edits to the tool call that created them.
	 */
...
export interface IChatSessionMetadataStore {
	...
	/**
	 * Copy all BatikCode-specific metadata (workspace info, request details, etc.) from
