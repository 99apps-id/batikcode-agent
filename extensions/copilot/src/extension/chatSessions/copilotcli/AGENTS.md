...
This folder contains the Copilot CLI integration for BatikCode Chat. It enables users to open a new Chat window and interact with a Copilot CLI agent instance directly within BatikCode. **BatikCode provides the UI, Copilot CLI SDK provides the smarts.**
...
│  - Provides BatikCode-specific tools to the SDK via MCP protocol   │
...
The integration follows BatikCode's platform layering pattern with three layers:

```
copilotcli/
├── common/                     # Platform-agnostic (NO Node.js or BatikCode API imports)
...
│   ├── ripgrepShim.ts          # Copies BatikCode's ripgrep for SDK use
│   └── test/
│
└── vscode-node/                # BatikCode API-dependent (commands, MCP tools, UI)
...
- Registers a `LanguageModelChatProvider` with `targetChatSessionType: 'copilotcli'` so BatikCode's model picker shows CLI models
...
- Listens to SDK events and translates them to BatikCode chat UI parts
...
- Proxies all BatikCode-configured MCP servers through a gateway URL with `type: 'http'` config per server
...
Persists BatikCode-specific metadata that sits alongside the SDK's own session data. This metadata is **not part of the SDK's `events.jsonl`** — it tracks BatikCode concepts like worktree properties, request-to-tool mappings, mode instructions, and checkpoint refs.
...
**BatikCode Session commands** (registered via `registerCLIChatCommands` in `vscode-node/copilotCLIChatSessions.ts`):

## Configuration

The integration respects these BatikCode settings (all under `github.copilot.chat.cli.*`):
...
| `showExternalSessions` | `false` | Show sessions created outside BatikCode (e.g., terminal CLI) |
