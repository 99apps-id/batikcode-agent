...
/**
 * Copies the node-pty files from BatikCode's installation into a @github/copilot location.
 *
 ...
 * Code's app tree). For built-in installs `require('node-pty')` walks up into BatikCode's
 * own `node_modules` and this shim is a no-op (the `shims.txt` placeholder skips it). For
 * marketplace installs we copy BatikCode's pre-built `pty.node` into the SDK's expected
 ...
 * @param vscodeAppRoot BatikCode's installation path (where node-pty is located)
 */
...
export async function resolveNodePtySourcePath(vscodeAppRoot: string, logService: ILogService): Promise<string> {
	// In a packaged build BatikCode's `node_modules` is bundled into a
