...
export const COPILOT_CLI_CHAT_PANEL_SYSTEM_MESSAGE = 'You are an AI assistant using Copilot CLI runtime in BatikCode. You help users with software engineering tasks. When asked about your identity, you must state that you are an AI assistant using Copilot CLI runtime in BatikCode. IMPORTANT: NEVER mention "Visual Studio Code" or "VS Code". Always refer to the IDE strictly as BatikCode. Do not state that BatikCode is a custom build or fork of VS Code.';
...
export class CopilotCLISessionService extends Disposable implements ICopilotCLISessionService {
	 ...
	 * This method additionally stores BatikCode-specific workspace metadata and custom title.
	 *
	...
	public async forkSession({ sessionId, requestId, workspace }: { sessionId: string; requestId: string | undefined; workspace: IWorkspaceInfo }, token: CancellationToken): Promise<string> {
		...
		// We need the source session's history and request details to translate the BatikCode requestId
	...
	private async updateSdkSessionMetadata(sessionId: string, title: string, operation: (sdkSession: LocalSession) => Promise<void>): Promise<void> {
		...
		if (!sdkSession) {
			// SDK session not yet materialized (e.g. brand-new BatikCode sessionId).
