...
interface McSharedState {
	...
	/** BatikCode session resource URI for routing steering through the chat UI. */
...
export class CopilotCLISession extends DisposableStore implements ICopilotCLISession {
	...
	private async _handleRequestImplInner(
		...
		// and therefore never reach the tools service). MCP/BatikCode tools already emit `execute_tool`
		...
		try {
			...
			for (const cmd of commands) {
				...
				switch (cmd.type) {
					...
					case 'user_message':
					default: {
						// Route steering messages through the BatikCode chat UI so
	 ...
	 * chat debug logs view. MCP/BatikCode tools (those carrying an `mcpServerName`) already emit spans
	 ...
	 * status. No-op for tools that were not synthesized (e.g. MCP/BatikCode tools).
	 */
