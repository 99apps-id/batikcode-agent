...
describe('CopilotCLISessionService', () => {
	...
	describe('CopilotCLISessionService.createSession', () => {
		...
		it('passes the BatikCode Copilot CLI identity system message to session manager', async () => {
			const createSessionSpy = vi.spyOn(manager, 'createSession');
			await service.createSession({ model: 'gpt-test', ...sessionOptionsFor(URI.file('/tmp')) }, CancellationToken.None);
		...
		it('preserves prompt variable context separately from the BatikCode Copilot CLI identity system message', async () => {
	...
	describe('CopilotCLISessionService.getSession', () => {
		...
		it('passes the BatikCode Copilot CLI identity system message when getting an existing session', async () => {
	...
	describe('CopilotCLISessionService.renameSession', () => {
		it('renames an inactive session through copilot/sdk', async () => {
			...
			await service.renameSession(sessionId, 'Renamed From BatikCode');

			expect(manager.sessions.get(sessionId)?.title).toBe('Renamed From BatikCode');
			expect(await service.getSessionTitle(sessionId, CancellationToken.None)).toBe('Renamed From BatikCode');
		});
