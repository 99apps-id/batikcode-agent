...
describe('ChatSessionInitializer', () => {
	...
	describe('resolveModelId', () => {
		...
		it('rejects BatikCode Provider Hub models instead of silently using the Copilot default', async () => {
			const { initializer } = createInitializer();
			const request = makeRequest({
				model: { id: 'nvidia:nvidia-nemotron', name: 'NVIDIA Nemotron', vendor: 'batikcode' }
			} as Partial<vscode.ChatRequest>);

			await expect(initializer.resolveModel(request, CancellationToken.None)).rejects.toThrow(
				'belongs to BatikCode Provider Hub'
			);
		});
