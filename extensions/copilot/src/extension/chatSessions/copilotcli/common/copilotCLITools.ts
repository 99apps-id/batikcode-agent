...
/**
 * Build chat history from SDK events for BatikCode chat session
 ...
 * BatikCode {@link McpToolInvocationContentData}[] objects for rendering in the chat UI.
 *
 ...
 * content shapes into a common binary+MIME-type representation that the BatikCode chat
...
export function createCopilotCLIToolInvocation(data: {
	...
	if (toolCall.toolName === 'show_file') {
		...
		// Its a WIP, no clear way to render in CLI either, hence decided to hide in BatikCode.
		return undefined;
	}
