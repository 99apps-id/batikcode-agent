...
/**
 * Converts a Claude Code session into BatikCode chat history turns.
 *
 ...
 * appears as alternating assistant/user messages in the JSONL. BatikCode's chat
...
export function buildChatHistory(session: IClaudeCodeSession, getModelDetails?: (modelId: string) => string | undefined): (vscode.ChatRequestTurn2 | vscode.ChatResponseTurn2)[] {
	...
	while (i < messages.length) {
		...
		if (currentType === 'user') {
			...
			// as a separate ChatResponseTurn2 because the BatikCode chat widget
