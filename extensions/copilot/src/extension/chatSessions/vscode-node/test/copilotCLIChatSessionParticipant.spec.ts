...
// The workaround tests override this to simulate the full BatikCode core round-trip.
...
function createChatContext(sessionId: string, isUntitled: boolean, ...requests: TestChatRequest[]): vscode.ChatContext {
	// Mirror the production invariant enforced by `isUntitledChatSession` in
	// `vs/workbench/contrib/chat/common/chatUri.ts`: an untitled chat session's
	// resource path always starts with `/untitled-`. The participant relies on
...
describe('CopilotCLIChatSessionParticipant.handleRequest', () => {
	...
	beforeEach(async () => {
		...
		// By default, simulate BatikCode core opening the delegated session and
		...
		sdk = {
			getPackage: vi.fn(async () => ({ internal: { LocalSessionManager: MockCliSdkSessionManager, NoopTelemetryService: class { } }, createLocalFeatureFlagServiceCreator, AutoModeSessionManager: class { }, noopTelemetryBinder: {} })),
	...
	it('hydrates invalid sessions from partial history and blocks follow-up requests', async () => {
		...
		const request = new TestChatRequest('Continue from BatikCode');
		const context = createChatContext(sessionId, false, request);
		const stream = new MockChatResponseStream();
	...
	it('starts a new chat session and submits the request', async () => {
		...
		// The request is processed asynchronously when BatikCode opens the session.
	...
	describe('chatSessionContext lost workaround (core bug)', () => {
		...
		// 3. BatikCode core opens the new session and calls handleRequest again with the copilotcli:// resource,
		...
		beforeEach(() => {
			...
			// Override the default round-trip behavior to simulate BatikCode core
			// calling handleRequest again with the copilotcli:// resource but with chatSessionContext lost.
			mockExecuteCommand.mockImplementation(async (command: string, args: any) => {
				if (command === 'workbench.action.chat.openSessionWithPrompt.copilotcli') {
					// Simulate BatikCode core: it opens the session and fires handleRequest,
		...
		it('full delegation round-trip: executeCommand triggers callback that uses workaround to reconstruct context and reuse session', async () => {
			...
			// The mock executeCommand simulates BatikCode calling handleRequest again with
