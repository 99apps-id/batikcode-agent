 ...
 * any other surface — `copilot-developer-cli` (Copilot CLI), `vscode-chat` (BatikCode) or
 ...
 * (Copilot CLI / BatikCode / JetBrains). The owning surface is identified by the agent integration
 ...
 * how we recover `{owner, name}` for {@link CloudSessionData.repo} and the resolver fallback.
 ...
 */
function taskToPullArtifactRef(task: AgentTask): PullArtifactRef | undefined {
