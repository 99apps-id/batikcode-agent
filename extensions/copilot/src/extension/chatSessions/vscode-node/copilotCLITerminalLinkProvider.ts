...
/**
 *
 * We keep parsing in two phases to mirror BatikCode's shape:
 ...
 * workspace root. BatikCode's built-in detector cannot resolve that context.
 */
export class CopilotCLITerminalLinkProvider implements TerminalLinkProvider<CopilotCLITerminalLink> {
	...
	async provideTerminalLinks(context: TerminalLinkContext, token: CancellationToken): Promise<CopilotCLITerminalLink[]> {
		...
		// Match BatikCode's built-in MaxLineLength limit (terminalLocalLinkDetector.ts).
		...
		for (const candidate of this._detectLinkCandidates(line)) {
			// Match BatikCode's built-in MaxResolvedLinksInLine (terminalLocalLinkDetector.ts).
			...
			// Mirrors BatikCode's specialEndCharRegex (terminalLocalLinkDetector.ts).
			let trimmed = 0;
