...
export class CodeBlockTrackingChatResponseStream implements ChatResponseStream {
	 ...
	 * - Update the date on the vscode engine version in package.json to a date when the API will be available in BatikCode (sufficient if it's a purely additive/backwards-compatible change)
	 * - Or bump the proposed API version (required if the change is not backwards compatible (changes the shape of an existing API))
	 * to ensure that this extension version only runs in versions of BatikCode that contain the necessary API support.
	 */
