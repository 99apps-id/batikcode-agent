...
A template for creating a BatikCode extension using Yeoman and Generator-Code.
...
5. Install any required BatikCode extensions based on the chosen language (e.g., Python extension for Python projects).
6. Inform the user that they can now debug this MCP server using BatikCode.
...
1. Call the tool `copilot_runVscodeCommand` to correctly create a new Python script project in BatikCode. Call the command with the following arguments.
...
1. Call the tool `run_vscode_command` to correctly create a new Python package project in BatikCode. Call the command with the following arguments.
