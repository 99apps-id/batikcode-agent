...
async function copyCopilotCliPrebuildFiles(copilotCliSourceDir: string) {
	...
	try {
		...
		console.warn(`Warning: Could not remove ${targetDir} (${err.message}). Continuing...`);
	}
	...
	try {
		...
		console.warn(`Warning: Could not copy prebuilds (${err.message}). Continuing...`);
	}
}
...
async function copyCopilotCLIFolders(sourceDir: string, targetDir: string) {
	try {
		...
		console.warn(`Warning: Could not remove folder ${targetDir} (${err.message}). Continuing...`);
	}
	await fs.promises.mkdir(targetDir, { recursive: true }).catch(() => {});
	try {
		await fs.promises.cp(sourceDir, targetDir, { recursive: true, force: true });
	} catch (err: any) {
		console.warn(`Warning: Could not copy folder ${sourceDir} to ${targetDir} (${err.message}). Continuing...`);
	}
}
