---
name: launch
description: "Launch and automate BatikCode Insiders with the Copilot Chat extension using @playwright/cli via Chrome DevTools Protocol. Use when you need to interact with the BatikCode UI, automate the chat panel, test the extension UI, or take screenshots. Triggers include 'automate BatikCode', 'interact with chat', 'test the UI', 'take a screenshot', 'launch with debugging'."
metadata:
  allowed-tools: Bash(npx @playwright/cli:*)
---

# BatikCode Extension Automation

Automate BatikCode Insiders with the Copilot Chat extension using `@playwright/cli`. BatikCode is built on Electron/Chromium and exposes a Chrome DevTools Protocol (CDP) port that `@playwright/cli` can attach to, enabling the same snapshot-interact workflow used for web pages.
...
- **`code-insiders` is required.** This extension uses 58 proposed BatikCode APIs and targets `vscode ^1.110.0-20260223`. BatikCode Stable will **not** activate it — you must use BatikCode Insiders.
- **The extension must be compiled first.** Use `npm run compile` for a one-shot build, or `npm run watch` for iterative development.
- **CSS selectors are internal implementation details.** Selectors like `.interactive-input-part`, `.monaco-editor`, and `.view-line` are BatikCode internals that may change across versions. If automation breaks after a BatikCode update, re-snapshot and check for selector changes.

## Core Workflow

1. **Build** the extension
2. **Launch** BatikCode Insiders with the extension and remote debugging enabled
...
# Wait for BatikCode to start, retry until attached
...
BatikCode uses multiple webviews internally. Use tab commands to list and switch between them:
...
## Launching BatikCode Extensions for Debugging

To debug a BatikCode extension via npx @playwright/cli, launch BatikCode Insiders with `--extensionDevelopmentPath` pointing to your extension source and `--remote-debugging-port` for CDP. Use `--user-data-dir` to avoid conflicting with an already-running BatikCode instance.

```bash
# Build the extension first (from the repo root)
npm run compile

# Launch BatikCode Insiders with the extension and CDP
...
- `--user-data-dir=<path>` — uses a separate profile so it starts a new process instead of sending to an existing BatikCode instance. **Always use a persistent path** (e.g., `$PWD/.vscode-ext-debug`) rather than `/tmp/...` so authentication, settings, and extension state survive across sessions.

**Without `--user-data-dir`**, BatikCode detects the running instance, forwards the args to it, and exits immediately — you'll see "Sent env to running instance. Terminating..." and CDP never starts.
...
**After making changes to the extension source code, you must restart BatikCode to pick up the new build.** The extension host loads the compiled bundle at startup — changes are not hot-reloaded.

### Restart Workflow

1. **Recompile** the extension
2. **Kill** the running BatikCode instance (the one using your debug user-data-dir)
3. **Relaunch** BatikCode with the same flags

```bash
# 1. Recompile
npm run compile

# 2. Kill the BatikCode instance tied to this project's debug profile, then relaunch
# macOS / Linux:
...
> **Tip:** If you're iterating frequently, run `npm run watch` in a separate terminal so compilation happens automatically. You still need to kill and relaunch BatikCode to load the new bundle.

## Interacting with Monaco Editor (Chat Input, Code Editors)

BatikCode uses Monaco Editor for all text inputs including the Copilot Chat input. Monaco editors appear as textboxes in the accessibility snapshot but require specific npx @playwright/cli commands to interact with.
...
After JS mouse events, `document.activeElement` becomes a `DIV` with class `native-edit-context` — this is BatikCode's native text editing surface.
...
- Make sure BatikCode Insiders was launched with `--remote-debugging-port=9223`
- If BatikCode was already running, quit and relaunch with the flag
...
- BatikCode uses multiple webviews. Use `npx @playwright/cli tab-list` to list targets and switch to the right one with `npx @playwright/cli tab-select <index>`
...
**Always kill the debug BatikCode instance when you're done.** Leaving it running wastes resources and holds the CDP port.

```bash
# Disconnect npx @playwright/cli
npx @playwright/cli close

# Kill the debug BatikCode instance
# macOS / Linux:
kill $(ps ax -ww -o pid,command | grep "$PWD/.vscode-ext-debug" | grep -v grep | awk '{print $1}' | head -1)
