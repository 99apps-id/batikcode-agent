...
You can run the BatikCode extension tests by running the `Markdown Extension Tests` target in BatikCode. This will run the tests under `./src/test`
...
This extension depends on `vscode-markdown-languageserver` and connects to it from `src/client/`. The code in `src/languageFeatures/` provides the BatikCode-specific glue and the features that are not handled by the server.
