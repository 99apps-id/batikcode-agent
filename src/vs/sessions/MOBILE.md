...
| "Open in BatikCode" action | ❌ Gated | `when: !sessionsIsPhoneLayout` on menu item |
...
Mobile picker subclasses live in `contrib/` alongside their base classes (not in `browser/parts/mobile/`), because BatikCode's layering rules prohibit `browser/` from importing `contrib/`. Each subclass extends the desktop picker and overrides the `_showPicker()` method to use `showMobilePickerSheet()` on phone, falling back to `super._showPicker()` on desktop. This means:
