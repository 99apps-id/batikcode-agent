...
The Agents Window workbench (`Workbench` in `sessions/browser/workbench.ts`) provides a simplified, fixed layout optimized for agent session workflows. Unlike the default BatikCode workbench, this layout:
...
| Right | `Menus.TitleBarRightLayout` | Remote connections, run script (split button), Open Terminal/BatikCode, toggle auxiliary bar, account widget |
