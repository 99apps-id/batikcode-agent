...
`events.jsonl` is the **primary** source and answers almost every question on its own. A *separate* log captures the **transport/protocol** between BatikCode and the agent host process — the JSON-RPC-style frames that drive sessions.
...
**It is written by BatikCode on the _client_ machine, so it is only reachable for a _local_ agent host** (BatikCode and the agent on the same machine). For a remote agent host it lives on the client, not the host this skill runs on — skip it there.

### Location

```
<BatikCode user-data dir>/logs/<session-timestamp>/ahp/ahp-<timestamp>-<connectionId>.jsonl
```

The BatikCode user-data dir depends on the build:
- Windows: `%APPDATA%\Code` (Insiders: `Code - Insiders`; OSS/dev may use `Code - OSS` or a custom `--user-data-dir`)
- macOS: `~/Library/Application Support/Code`
- Linux: `~/.config/Code`

A new `logs/<timestamp>/` folder is created per BatikCode session — pick the **most recently modified** `ahp/ahp-*.jsonl`.
...
**If no `ahp/` folder exists, the wire log is disabled** (it is off by default). Tell the user to set `"chat.agentHost.ahpJsonlLoggingEnabled": true`, restart BatikCode, reproduce the issue, then re-run.
...
- `_ahpLog.dir` — direction: `c2s` = BatikCode → host (requests/notifications), `s2c` = host → BatikCode (results/errors/actions/notifications).
