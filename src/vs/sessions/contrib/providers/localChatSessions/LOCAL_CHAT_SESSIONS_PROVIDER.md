...
The local sessions provider, registered with ID `'local-chat'`. Wraps local in-process BatikCode chat sessions (created via `IChatService.startNewLocalSession()`) into the extensible sessions provider model. Owns its own session list — does not rely on the chat history APIs at runtime.
