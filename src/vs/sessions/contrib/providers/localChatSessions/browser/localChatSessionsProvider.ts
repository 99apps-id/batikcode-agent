...
/** Local session type — in-process BatikCode chat, no background agent or worktree. */
...
/** Setting key controlling whether Local BatikCode chat sessions are available in the Agents app. */
...
export class LocalChatSessionsProvider extends Disposable implements ISessionsProvider {
	...
	getModelsSnapshot(_sessionId: string, desiredModelId?: string): ISessionModelsSnapshot {
		// Local (in-process BatikCode chat) sessions use general-purpose models
	...
	getModelPickerOptions(_sessionId: string): ISessionModelPickerOptions {
		// Local (in-process BatikCode chat) sessions offer the "Manage Models"
