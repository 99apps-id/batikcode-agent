...
suite('RemoteAgentHostCustomizationHarness', () => {
	...
	test('provider hides synthetic bundle but still expands its contents', async () => {
		...
		const bundleRef: Customization = { type: CustomizationType.Plugin, id: bundleUri, uri: bundleUri, name: 'BatikCode Synced Data', enabled: true, load: { kind: CustomizationLoadStatus.Loaded } };
		const synced: Customization = {
			...bundleRef,
			clientId: 'test-client',
		};
		...
		assert.ok(!items.some(i => i.name === 'BatikCode Synced Data'), 'synthetic bundle should be hidden');
	...
	test('toRemoteUri preserves synced-customization scheme URIs', async () => {
		...
		const bundleUri = `${SYNCED_CUSTOMIZATION_SCHEME}:///test-authority`;
		const bundleRef: Customization = { type: CustomizationType.Plugin, id: bundleUri, uri: bundleUri, name: 'BatikCode Synced Data', enabled: true };
		const synced: Customization = {
			...bundleRef,
			clientId: 'test-client',
		};
	...
	test('provider recovers original provenance for synthetic-bundle children via the origin resolver', async () => {
		...
		// The synthetic "BatikCode Synced Data" bundle lives under the synced scheme.
		const bundleUri = `${SYNCED_CUSTOMIZATION_SCHEME}:///test-authority`;
		const bundle: Customization = { type: CustomizationType.Plugin, id: bundleUri, uri: bundleUri, name: 'BatikCode Synced Data', enabled: true };
