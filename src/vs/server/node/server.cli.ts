...
/*
 * Implements a standalone CLI app that opens BatikCode from a remote terminal.
 ...
 *  - In external terminals for WSL this calls BatikCode on the Windows side.
 *    The BatikCode desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
export async function main(desc: ProductDescription, args: string[]): Promise<void> {
	if (!cliPipe && !cliCommand) {
		console.log('Command is only available in WSL or inside a BatikCode terminal.');
		return;
	}
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to BatikCode server: ' + message);
	console.error(err);
	process.exit(1);
}
