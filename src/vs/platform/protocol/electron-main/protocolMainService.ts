...
export class ProtocolMainService extends Disposable implements IProtocolMainService {
	...
	private readonly validExtensions = new Set(['.svg', '.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp', '.mp4', '.otf', '.ttf', '.css', '.js', '.map', '.json', '.wasm', '.woff', '.woff2', '.html']); // https://github.com/microsoft/vscode/issues/119384
	...
	addValidFileRoot(root: string): IDisposable {
		...
		if (!this.validRoots.get(normalizedRoot)) {
			...
			let altDrive: string | undefined;
			if (!isLinux && normalizedRoot.length >= 2 && normalizedRoot[1] === ':') {
				const firstChar = normalizedRoot[0];
				altDrive = (firstChar.toLowerCase() === firstChar ? firstChar.toUpperCase() : firstChar.toLowerCase()) + normalizedRoot.slice(1);
				this.validRoots.set(altDrive, true);
			}

			return toDisposable(() => {
				this.validRoots.delete(normalizedRoot);
				if (altDrive) {
					this.validRoots.delete(altDrive);
				}
			});
		}
