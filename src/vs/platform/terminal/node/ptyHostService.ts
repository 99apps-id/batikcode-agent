...
export class PtyHostService extends Disposable implements IPtyHostService {
		...
		store.add(connection.onDidProcessExit(e => {
			...
			if (!this._wasQuitRequested && !this._store.isDisposed) {
				if (this._restartCount <= Constants.MaxRestarts) {
					...
					// Do not retain a proxy to the dead utility process. A later
					// terminal request can then start a fresh host after a transient
					// failure, for example once a development rebuild has restored
					// the runtime files.
	...
	private _handleHeartbeat(isConnecting?: boolean) {
		this._clearHeartbeatTimeouts();
		if (!isConnecting) {
			// A real heartbeat proves the replacement host is healthy. Reset the
			// consecutive-crash budget so a recovered host is not permanently one
			// failure away from the "giving up" state.
