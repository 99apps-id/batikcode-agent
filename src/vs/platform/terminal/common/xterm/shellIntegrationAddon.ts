...
export const enum ShellIntegrationOscPs {
	...
	FinalTerm = 133,
	/**
	 * Sequences pioneered by BatikCode. The number is derived from the least significant digit of
...
/**
 * BatikCode-specific shell integration sequences. Some of these are based on more common alternatives
 ...
 * the terminal. If multiple shell integration scripts run, BatikCode will prioritize the VS
 * Code-specific ones.
 *
 * It's recommended that authors of shell integration scripts use the common sequences (`133`)
 * when building general purpose scripts and the BatikCode-specific (`633`) when targeting only VS
...
const enum VSCodeOscPt {
	...
	/**
	 * Set the value of an arbitrary property, only known properties will be handled by BatikCode.
	 *
	 ...
	 *   BatikCode knows the command line when the execution begins.
	 *
...
export class ShellIntegrationAddon extends Disposable implements IShellIntegration, ITerminalAddon {
	...
	private _doHandleFinalTermSequence(data: string): boolean {
		...
		// It was considered to disable the common protocol in order to not confuse the BatikCode
