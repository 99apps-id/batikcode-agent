...
suite('PtyHostService', () => {
	...
	test('a terminal request can recover after the pty host exhausts automatic restarts', async () => {
		let startCount = 0;
		let currentExitEmitter: Emitter<{ code: number; signal: string }> | undefined;
		...
		const starter: IPtyHostStarter = {
			start: (): IPtyHostConnection => {
				...
				currentExitEmitter = connectionStore.add(new Emitter<{ code: number; signal: string }>());
				return {
					client,
					store: connectionStore,
					onDidProcessExit: currentExitEmitter.event
				};
			},
			dispose: () => { }
		};
		...
		await service.restartPtyHost();
		for (let i = 0; i < 7; i++) {
			currentExitEmitter!.fire({ code: 1, signal: '' });
		}
		strictEqual(startCount, 7, 'the service should stop automatic restarts after exhausting its retry budget');

		await service.listProcesses();
		strictEqual(startCount, 8, 'a later terminal request should start a fresh pty host');
	});
});
