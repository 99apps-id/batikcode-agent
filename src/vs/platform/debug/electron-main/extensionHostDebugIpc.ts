		...
		const server = createServer((req, res) => {
			if (req.url === '/json/list' || req.url === '/json') {
				...
				res.end(JSON.stringify([{
					description: 'BatikCode Renderer',
					devtoolsFrontendUrl: '',
					id: ident,
					title: 'BatikCode Renderer',
					type: 'page',
					url: 'vscode://renderer',
		...
		const { server, wsUrl, port } = await this.openCdpServer(ident, listener => {
			...
			store.add(listener.onData(rawData => {
				...
				if (debugRenderer) {
					...
					const targetInfo = { targetId: ident, type: 'page', title: 'BatikCode Renderer', url: 'vscode://renderer' };
