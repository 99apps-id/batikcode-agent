 ...
 * using BatikCode's `${env:VAR}` colon-delimited syntax.
 */
const BARE_ENV_VAR_RE = /\$\{(?![A-Za-z]+:)([A-Z_][A-Z0-9_]*)\}/g;

/**
 * Converts bare `${VAR}` environment-variable references to BatikCode `${env:VAR}` syntax.
 */
 ...
 * `.instructions.md` for compatibility with BatikCode-discovered instructions
