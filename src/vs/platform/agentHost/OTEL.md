...
> **Activation timing.** Env vars are bound at agent host **spawn time**. Changing a setting while the agent host is already running has no effect until the host respawns — restart BatikCode or reload the window if you change these settings mid-session.
