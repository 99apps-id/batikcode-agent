 ...
 * CAPI proxy through BatikCode's Electron session before spawning the Copilot SDK.
 *
...
export interface IAgentHostProxyResolver {
	 ...
	 * so the same precedence as the rest of BatikCode applies: `http.noProxy` →
	 * `http.proxy` setting → `HTTP(S)_PROXY` env vars → the host proxy resolution
	 * that runs in BatikCode (Electron session) via the reverse channel.
	 */
	...
	private _getProxyResolver(): ReturnType<typeof createProxyResolver> {
		if (!this._proxyResolver) {
			...
			const params: ProxyAgentParams = {
				// The host proxy resolution runs in BatikCode: reverse-call a connected
