...
/** BatikCode `http.*` proxy settings surfaced alongside the env vars. */
...
export class NetworkDiagnosticsService implements INetworkDiagnosticsService {
	 ...
	 * traverses the same proxy / TLS / certificate stack the rest of BatikCode
	...
	async fetch(url: string): Promise<IAgentHostNetworkFetchResult> {
		...
		// Reachability: a GET through IRequestService, which applies BatikCode's proxy,
		// strictSSL, and certificate handling — the path the rest of BatikCode uses.
