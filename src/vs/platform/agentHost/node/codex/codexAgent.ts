...
export class CodexAgent extends Disposable implements IAgent {
	...
	private async _startConnection(token: string): Promise<IConnectionReady> {
		...
		const providerOverrides = [
			...
			`model_providers.vscode-proxy.name="BatikCode Proxy"`,
	...
	private _registerIgnoredNotifications(client: ICodexAppServerClient): void {
		const ignored = [
			...
			'thread/settings/updated', // BatikCode owns session config; Codex settings echoes are not consumed yet.
			...
			'remoteControl/status/changed', // Remote-control state is not part of the BatikCode integration.
