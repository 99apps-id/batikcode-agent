...
export class CopilotAgent extends Disposable implements IAgent {
	...
	private async _ensureClient(): Promise<CopilotClient> {
		...
		const clientStarting = (async () => {
			...
			// Build a clean env for the CLI subprocess, stripping Electron/BatikCode vars
			...
			// Identify BatikCode's agent host traffic in CAPI
			...
			// (and the Linux/macOS equivalents). BatikCode core ships the MXC sandbox binaries
			...
			// Add BatikCode's built-in ripgrep to PATH so the CLI subprocess can find it.
