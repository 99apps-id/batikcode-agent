...
// bundled with BatikCode; users either install codex themselves (typically via
...
// BatikCode instead of leaking via shell env.
//
...
export interface IAgentHostNetworkDiagnosticsInfo {
	...
	/** BatikCode `http.*` proxy settings observed by the agent host, keyed by setting id (only those that are set). */
...
export interface IActiveClient {
	...
	/** Human-readable client name (e.g. `"BatikCode"`), if provided. */
