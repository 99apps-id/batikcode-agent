...
This directory contains the BatikCode-facing wrappers around the Agent Host
...
- `sessionState.ts`, `sessionActions.ts`, `sessionReducers.ts`, and
  `sessionProtocol.ts` are BatikCode-facing wrappers and re-exports.
- `protocol/**` is generated from the sibling `agent-host-protocol` repo by
  ...
  banner; update the source protocol repo and sync the copy into BatikCode.
...
- Session teardown uses `changeset/cleared` plus the corresponding
  ...
  action in the BatikCode protocol copy.

## Updating generated protocol types

1. Update the source files in the sibling `agent-host-protocol` repo.
2. Run `npx tsx scripts/sync-agent-host-protocol.ts` from the BatikCode repo.
3. If BatikCode consumers need short aliases or type guards, update the wrapper
...
4. Re-export the new action from `sessionActions.ts` when BatikCode callers need
   the type directly.
