...
/**
 * Root config key forwarded from the renderer when BatikCode's
...
/**
 * The BatikCode setting ID for terminal auto approve enablement. Defined here so
...
/**
 * The BatikCode setting ID for global auto approve. Defined here so renderer-side
...
/**
 * Root config key forwarded from the renderer when BatikCode's `chat.autoReply`
...
/**
 * The BatikCode setting ID for auto-reply. Defined here so renderer-side
 * agent-host clients can forward it without importing from `workbench/contrib/chat`.
 */
...
/**
 * The BatikCode setting IDs for terminal auto approve rules. Defined here so
...
/**
 * The BatikCode setting ID for session sync. Defined here so the platform
...
export const platformRootSchema = createSchema({
	...
	[AgentHostGlobalAutoApproveEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.globalAutoApproveEnabled.description', "Whether BatikCode's global auto-approve setting is enabled. When `true`, every tool call is auto-approved, equivalent to a session using Allow all."),
		default: false,
	}),
	[AgentHostAutoReplyEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.autoReplyEnabled.description', "Whether BatikCode's auto-reply setting is enabled. When `true`, `ask_user` questions are auto-answered instead of blocking on the user, mirroring autopilot mode."),
		default: false,
	}),
