...
> These are **e2e tests**. The `*.integrationTest.ts` file suffix and `test-integration.sh` script are just the BatikCode test-runner conventions they hook into.
...
- **Isolated persistent state**: each shared test uses a temporary home and BatikCode user-data directory, which teardown removes after the agent host exits.
