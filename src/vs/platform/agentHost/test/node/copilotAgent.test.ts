...
import type { CopilotClient, CopilotClientOptions, CopilotSession, GitHubTelemetryNotification, ModelInfo, PermissionAllowAllMode, PermissionRequest, SessionEvent, SessionEventHandler, SessionEventPayload, SessionEventType, TypedSessionEventHandler } from '@github/copilot-sdk';
...
suite('CopilotAgent', () => {
	...
	suite('provisional sessions', () => {
		...
		test('materialization passes BatikCode-specific system message to the SDK', async () => {
			const sessionDataService = disposables.add(new TestSessionDataService());
			const client = new TestCopilotClient([]);
			...
			try {
				...
				assert.strictEqual(
					...
					'You are an AI assistant using Copilot CLI runtime in BatikCode. You help users with software engineering tasks. When asked about your identity, you must state that you are an AI assistant using Copilot CLI runtime in BatikCode.'
				);
			} finally {
				await disposeAgent(agent);
			}
