...
The current authentication mechanism is imperative and BatikCode-specific:

1. The renderer discovers agents via `listAgents()` and checks `IAgentDescriptor.requiresAuth`.
2. It obtains a GitHub OAuth token from BatikCode's built-in authentication service.
...
### Client-side (BatikCode renderer)
...
2. **Phase B**: Update BatikCode renderer to use `authenticate` instead of `setAuthToken`. External clients can start using the new flow.
