...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'BatikCode', separator: { label: ' - ' } }), 'BatikCode');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'BatikCode', separator: { label: ' - ' } }), 'Untitled-1 - BatikCode');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'BatikCode', separator: { label: ' - ' } }), 'monaco - BatikCode');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'BatikCode', separator: { label: ' - ' } }), 'somefile.txt - monaco - BatikCode');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'BatikCode', separator: { label: ' - ' } }), '* somefile.txt - monaco - BatikCode');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'BatikCode', separator: { label: ' - ' } }), 'monaco - BatikCode');
	});
