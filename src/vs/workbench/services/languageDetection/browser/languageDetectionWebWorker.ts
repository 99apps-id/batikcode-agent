...
export class LanguageDetectionWorker implements ILanguageDetectionWorker {
	...
	private adjustLanguageConfidence(modelResult: ModelResult): ModelResult {
		switch (modelResult.languageId) {
			...
			// these are commonly used languages in BatikCode and supported
			// by the model.
			case 'js':
			case 'html':
			case 'json':
			...
			// languages that are provided by default in BatikCode
			case 'bat':
			case 'ini':
			case 'makefile':
			case 'sql':
			// languages that aren't provided by default in BatikCode
			case 'csv':
			case 'toml':
