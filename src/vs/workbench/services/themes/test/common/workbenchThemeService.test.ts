...
suite('WorkbenchThemeService', () => {
	...
	suite('migrateThemeSettingsId', () => {
		...
		test('migrates Experimental theme IDs to BatikCode themes', () => {
			assert.deepStrictEqual(
				['Experimental Dark', 'Experimental Light', 'BatikCode Dark', 'BatikCode Light'].map(migrateThemeSettingsId),
