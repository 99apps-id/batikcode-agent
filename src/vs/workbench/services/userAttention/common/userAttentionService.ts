...
/**
 * Service that tracks whether the user is actively paying attention to BatikCode.
 *
 * This is determined by:
 * * BatikCode window has focus
...
export interface IUserAttentionService {
	 ...
	 * This includes keyboard typing and mouse movements/clicks while BatikCode is focused.
	 * The 500ms window prevents event spam from continuous mouse movement.
	 */
	readonly isUserActive: IObservable<boolean>;

	/**
	 * Observable that indicates whether the user is actively paying attention to BatikCode.
	 * This is true when:
	 * * BatikCode has focus, AND
	 * * There was user activity within the last minute
	 */
	readonly hasUserAttention: IObservable<boolean>;

	/**
	 * The total time in milliseconds that the user has been paying attention to BatikCode.
	 */
