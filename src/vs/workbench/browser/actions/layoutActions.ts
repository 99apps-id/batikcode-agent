...
export class ToggleSidebarVisibilityAction extends Action2 {
	...
	static readonly LABEL = localize('compositePart.focusSideBarLabel.batikCode', "Focus Primary Side Bar");

	constructor() {
		super({
			id: ToggleSidebarVisibilityAction.ID,
			title: localize2('focusSidebar.batikCode', 'Focus Primary Side Bar'),
			metadata: {
				description: localize('focusAlwaysVisibleSidebar.batikCode', 'Focus the Primary Side Bar, which is always visible in BatikCode.'),
			},
			category: Categories.View,
			f1: true,
			keybinding: {
	...
	run(accessor: ServicesAccessor): void {
		...
		alert(localize('sidebarFocused.batikCode', "Primary Side Bar focused"));
	}
}
