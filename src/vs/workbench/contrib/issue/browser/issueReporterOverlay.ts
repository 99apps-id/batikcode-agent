...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate BatikCode and choose when to capture.", MAX_ATTACHMENTS);
	...
	private createFloatingCaptureBar(): void {
		...
		// Mount inside .monaco-workbench so BatikCode's color theme CSS vars
		...
		// Delay/options dropdown using BatikCode's context menu
	...
	private createStep1Describe(): void {
		...
		// BatikCode (the most common target). Extension is preselected only when an
	...
	private getIssueTypeOptions(): { type: IssueType; label: string; icon: { id: string } }[] {
		...
		// itself, where performance metrics from a single BatikCode instance aren't useful.
	...
	private getAllSourceOptions(): { label: string; value: IssueSource }[] {
		return [
			{ label: product.nameLong || localize('vscode', "BatikCode"), value: IssueSource.VSCode },
			{ label: localize('agentsWindow', "Agents Window"), value: IssueSource.AgentsWindow },
			{ label: localize('extensionSource', "A BatikCode extension"), value: IssueSource.Extension },
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			case IssueSource.VSCode:
				return product.nameLong || localize('vscode', "BatikCode");
			case IssueSource.AgentsWindow:
				return localize('agentsWindow', "Agents Window");
			case IssueSource.Extension:
				return this.selectedExtension?.displayName || this.selectedExtension?.name || localize('extensionSource', "A BatikCode extension");
			case IssueSource.Marketplace:
	...
	private updateReviewDetails(): void {
			...
			this.createDiagSection(diagContainer, {
				...
				renderContent: (container) => {
					...
					if (modelData.versionInfo) {
						this.addDiagRow(sysTable, 'BatikCode', modelData.versionInfo.vscodeVersion);
		...
		if (modelData.extensionData) {
			...
			// Extension (e.g. built-in extensions are filed against BatikCode),
	...
	private generateIssueDetailsMd(): string {
		...
		const rows: [string, string | undefined][] = [
			...
			['BatikCode Version', modelData.versionInfo?.vscodeVersion ?? product.version],
	...
	private generateSystemInfoMd(): string {
		...
		if (modelData.versionInfo) {
			rows.push(
				['BatikCode Version', modelData.versionInfo.vscodeVersion],
