...
The core editor in BatikCode is packed with features. This page highlights a number of them and lets you try them interactively through embedded editors. For BatikCode-specific guidance, visit the [project documentation](https://github.com/99apps-id/batikcode/tree/main/docs).
...
BatikCode includes powerful IntelliSense for JavaScript and TypeScript. In the example below, position the text cursor right after the dot and press kb(editor.action.triggerSuggest) to invoke IntelliSense. Notice how the suggestions come from the Canvas API.
...
> **JSDoc Tip:** BatikCode's IntelliSense uses JSDoc comments to provide richer suggestions. Types and documentation from JSDoc appear when you hover over |Book| or create a new |Book| instance.
...
>**Tip:** The [Emmet cheat sheet](https://docs.emmet.io/cheat-sheet/) is a useful source of syntax suggestions. To expand Emmet abbreviations and snippets with |tab|, enable the |emmet.triggerExpansionOnTab| [setting](command:workbench.action.openGlobalSettings).
...
>**Tip:** You can also enable checks workspace- or application-wide by adding |"js/ts.implicitProjectConfig.checkJs": true| to your workspace or user settings, then ignore files or lines explicitly with |// @ts-nocheck| and |// @ts-expect-error|.


## Thanks!
If you have made it this far, you have tried several editing features in BatikCode. Continue with the [BatikCode documentation](https://github.com/99apps-id/batikcode/tree/main/docs). Here are a few more things you can try:
- Open the Integrated Terminal by pressing kb(workbench.action.terminal.toggleTerminal)
- Work with version control by pressing kb(workbench.view.scm)
- Browse extensions in the integrated Open VSX gallery by pressing kb(workbench.view.extensions)
