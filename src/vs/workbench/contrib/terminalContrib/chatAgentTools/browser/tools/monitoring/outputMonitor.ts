...
export class OutputMonitor extends Disposable implements IOutputMonitor {
	...
	private async _handleIdleState(token: CancellationToken): Promise<{ resources?: ILinkLocation[]; shouldContinuePolling: boolean; output?: string }> {
		...
		// Check for BatikCode's task finish messages (like "press any key to close the terminal").
		// If the execution is a task and the output contains a BatikCode task finish message,
		...
		if (isTask && detectsVSCodeTaskFinishMessage(outputTail)) {
			this._logService.trace('OutputMonitor: Idle -> BatikCode task finish message detected, stopping');
...
/**
 * Localized task finish messages from BatikCode's terminalTaskSystem.
...
/**
 * Detects BatikCode's specific task completion messages like:
 * - "Press any key to close the terminal."
 * - "Terminal will be reused by tasks, press any key to close it."
 * These appear when a task finishes and should be ignored if the task is done.
 * Note: These messages may be prefixed with " * " by BatikCode and may have line wrapping
...
/**
 * Detects generic "press any key" prompts from scripts (not BatikCode task messages).
...
export function detectsGenericPressAnyKeyPattern(cursorLine: string): boolean {
	// Match "press any key" but exclude BatikCode task-specific messages
