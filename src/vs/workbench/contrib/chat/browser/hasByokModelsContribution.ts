...
export class HasByokModelsContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._register(Event.any(
			...
			// A vendor resolving its models is the signal that BYOK is live for
			// providers that never write a configuration group.
			Event.map(this._languageModelsService.onDidChangeLanguageModels, () => undefined),
		)(() => this._update()));
	}
	...
	/** True when a vendor other than Copilot has resolved at least one model. */
	private _hasResolvedNonCopilotModels(): boolean {
		return this._languageModelsService.getVendors().some(vendor =>
			vendor.vendor !== COPILOT_VENDOR_ID
			&& this._languageModelsService.getLanguageModelGroups(vendor.vendor).some(group => group.modelIdentifiers.length > 0));
	}
	...
	private _update(): void {
		...
		// A provider extension can register its vendors and serve models without
		// ever writing a configuration group — the group list only covers setups
		// the user configured through the Manage Models view. Resolved models are
		// the authoritative signal, and unlike the user-selectable context key
		// they are read from the live cache rather than a value that can go stale.
