...
function getUpdateHoverContent(updateState: StateType): MarkdownString {
	...
	switch (updateState) {
		case StateType.AvailableForDownload:
			hoverContent.appendMarkdown(localize('chat.modelPicker.downloadUpdateHover', "This model requires a newer version of BatikCode. [Download Update](command:update.downloadUpdate) to access it."));
			break;
		case StateType.Downloaded:
		case StateType.Ready:
			hoverContent.appendMarkdown(localize('chat.modelPicker.restartUpdateHover', "This model requires a newer version of BatikCode. [Restart to Update](command:update.restartToUpdate) to access it."));
			break;
		default:
			hoverContent.appendMarkdown(localize('chat.modelPicker.checkUpdateHover', "This model requires a newer version of BatikCode. [Update BatikCode](command:update.checkForUpdate) to access it."));
			break;
	}
	return hoverContent;
}
...
function createUnavailableModelItem(
	...
	if (reason === 'upgrade') {
		...
		description = localize('chat.modelPicker.updateDescription', "Update BatikCode");
	} else {
