	...
	properties: {
		...
		[mcpAccessConfig]: {
			...
			enumDescriptions: [
				...
				nls.localize('chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that BatikCode is connected to."),
			...
			policy: {
				...
				localization: {
					...
					enumDescriptions: [
						...
						{
							key: 'chat.mcp.access.registry', value: nls.localize('chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that BatikCode is connected to."),
						},
						{
		...
		[ChatConfiguration.ExtraMarketplaces]: {
			...
			// JSON Schema's `type` keyword (equivalent to `type: 'string'`) to trigger BatikCode's
		...
		[AgentHostCopilotSdkLogLevelSettingId]: {
			...
			enumDescriptions: [
				nls.localize('chat.agentHost.copilotSdk.logLevel.info', "Log informational messages. Running BatikCode with trace logging still enables all Copilot SDK runtime diagnostics."),
