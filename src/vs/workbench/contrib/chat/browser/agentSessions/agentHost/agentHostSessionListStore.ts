...
export class AgentHostSessionListStore extends Disposable {
	...
	constructor(
		...
		// Re-fetch the session list whenever the set of BatikCode workspace
		// folders changes, since filtering depends on it. The agent host itself
		// doesn't know which workspace this BatikCode window has open.
	 ...
	 * to the current BatikCode workspace. When the window has no workspace
	 * folders open (e.g. the Agents window, or an empty BatikCode window),
