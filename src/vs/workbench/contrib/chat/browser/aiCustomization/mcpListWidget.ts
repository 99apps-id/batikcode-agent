	...
	private filterServers(): void {
		...
		if (extensionServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('extensionGroupDescription', "MCP servers contributed by installed BatikCode extensions."),
				collapsed,
			});
			if (!collapsed) {
		...
		if (otherBuiltinServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('builtInGroupDescription', "MCP servers built into BatikCode. These are available automatically."),
				collapsed,
			});
			if (!collapsed) {
