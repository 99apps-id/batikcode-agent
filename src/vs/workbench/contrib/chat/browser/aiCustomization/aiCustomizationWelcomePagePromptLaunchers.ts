	...
	private getPromptMigrationDescription(): string {
		...
		if (workspacePromptCount > 0 && userPromptCount > 0) {
			return localize(
				...
				"Prompt files are deprecated for this harness. Found {0} prompt files ({1} workspace, {2} global) that local BatikCode can still run, but {3} ignores. Convert them to skills to keep them available.",
				totalPromptCount,
		...
		if (workspacePromptCount > 0) {
			return localize(
				...
				"Prompt files are deprecated for this harness. Found {0} workspace prompt files that local BatikCode can still run, but {1} ignores. Convert them to skills to keep them available.",
				workspacePromptCount,
				this.harnessLabel,
			);
		}
		...
		return localize(
			...
			"Prompt files are deprecated for this harness. Found {0} global prompt files that local BatikCode can still run, but {1} ignores. Convert them to skills to keep them available.",
			userPromptCount,
			this.harnessLabel,
		);
	}
