	...
	private updatePromptMigrationPageDescription(): void {
		...
		if (workspacePromptCount > 0 && userPromptCount > 0) {
			this.migrationDescriptionElement.textContent = localize(
				...
				"Prompt files are not supported for this harness. Found {0} prompt files ({1} workspace, {2} user) that local BatikCode can still run, but {3} ignores. Convert them to skills to keep them available.",
				totalPromptCount,
		...
		if (workspacePromptCount > 0) {
			this.migrationDescriptionElement.textContent = localize(
				...
				"Prompt files are not supported for this harness. Found {0} workspace prompt files that local BatikCode can still run, but {1} ignores. Convert them to skills to keep them available.",
				workspacePromptCount,
				harnessLabel,
			);
			return;
		...
		this.migrationDescriptionElement.textContent = localize(
			...
			"Prompt files are not supported for this harness. Found {0} user prompt files that local BatikCode can still run, but {1} ignores. Convert them to skills to keep them available.",
			userPromptCount,
			harnessLabel,
		);
	}
