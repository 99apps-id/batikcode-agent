...
export function buildAutomationsHelpContent(keybindingService: IKeybindingService): string {
	...
	lines.push(nls.localize('automations.help.historyDesc', 'Each run records its status (Pending, Running, Completed, Failed), the trigger that started it (Schedule, Manual, or Catch-up after BatikCode restarts), the time it started, and the time it took. Failed runs include the failure reason.'));
	lines.push('');
