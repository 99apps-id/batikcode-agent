...
export class ChatSetupController extends Disposable {
	...
	private async doInstall(): Promise<void> {
		if (this.productService.defaultChatAgent?.skipSetup) {
			await this.extensionService.whenInstalledExtensionsRegistered();

			const bundledChatExtension = this.extensionService.extensions.find(extension =>
				ExtensionIdentifier.equals(extension.identifier, defaultChat.chatExtensionId)
			);
			if (!bundledChatExtension) {
				throw new Error(localize(
					'bundledChatExtensionMissing',
					"The built-in BatikCode chat extension '{0}' is unavailable.",
					defaultChat.chatExtensionId
				));
			}

			// BatikCode ships Copilot Chat as a built-in extension. Keep the normal
			// GitHub sign-in and entitlement flow, but never ask Open VSX to install
			// Microsoft's marketplace-only extension.
			await this.extensionService.activateByEvent('onLanguageModelChat:copilot', ActivationKind.Immediate);
			return;
		}
