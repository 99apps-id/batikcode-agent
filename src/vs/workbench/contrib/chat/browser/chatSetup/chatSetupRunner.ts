...
export class ChatSetup {
	...
	private getButtons(options?: IChatSetupRunOptions): Array<[string, ChatSetupStrategy, { styleButton?: (button: IButton) => void } | undefined]> {
		...
		if (!options?.forceAnonymous && (this.context.state.entitlement === ChatEntitlement.Unknown || options?.forceSignInDialog)) {
			...
			const googleProviderButton: ContinueWithButton | undefined = defaultChat.provider.google
				? [localize('continueWith', "Continue with {0}", defaultChat.provider.google.name), ChatSetupStrategy.SetupWithGoogleProvider, styleButton('continue-button', 'google')]
				: undefined;
			const appleProviderButton: ContinueWithButton | undefined = defaultChat.provider.apple
				? [localize('continueWith', "Continue with {0}", defaultChat.provider.apple.name), ChatSetupStrategy.SetupWithAppleProvider, styleButton('continue-button', 'apple')]
				: undefined;
