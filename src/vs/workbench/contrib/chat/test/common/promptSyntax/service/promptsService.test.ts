	...
	suite('getCustomAgents', () => {
		...
		test('header with target', async () => {
			...
			await mockFiles(fileService, [
				...
				},
				{
					...
					contents: [
						'---',
						'description: \'BatikCode specialized agent.\'',
						'target: \'vscode\'',
						'model: \'gpt-4\'',
						'---',
						'I am specialized for BatikCode editor tasks.',
					]
				},
				{
			...
			const expected: ICustomAgent[] = [
				...
				},
				{
					...
					model: ['gpt-4'],
					agentInstructions: {
						content: 'I am specialized for BatikCode editor tasks.',
						toolReferences: [],
						metadata: undefined
					},
					handOffs: undefined,
		...
		test('claude agent maps tools and model to vscode equivalents', async () => {
			...
			assert.deepEqual(
				result,
				expected,
				'Claude tools and models must be mapped to BatikCode equivalents; non-Claude agents must remain unchanged.',
			);
		});
