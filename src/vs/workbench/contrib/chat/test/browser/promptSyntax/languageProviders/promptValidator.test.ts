...
suite('PromptValidator', () => {
	...
	suite('agents', () => {
		...
		test('unknown attribute in agent file', async () => {
			...
			assert.deepStrictEqual(
				markers.map(m => ({ severity: m.severity, message: m.message, tags: m.tags })),
				[
					{ severity: MarkerSeverity.Hint, message: `Attribute 'applyTo' is not supported in BatikCode agent files. Supported: agents, argument-hint, description, disable-model-invocation, github, handoffs, hooks, model, name, target, tools, user-invocable.`, tags: [MarkerTag.Unnecessary] },
				]
			);
		});
		...
		test('vscode target agent validates normally', async () => {
			...
			const markers = await validate(content, PromptsType.agent);
			assert.deepStrictEqual(markers, [], 'BatikCode target should validate normally');
		});
		...
		test('vscode target agent with mcp-servers and github-tools', async () => {
			...
			const messages = markers.map(m => m.message);
			assert.deepStrictEqual(messages, [
				'Attribute \'mcp-servers\' is ignored when running locally in BatikCode.',
	...
	suite('claude rules', () => {
		...
		test('claude rules with unknown attribute shows unnecessary hint', async () => {
			...
			assert.strictEqual(markers[0].severity, MarkerSeverity.Hint);
			assert.deepStrictEqual(markers[0].tags, [MarkerTag.Unnecessary]);
			assert.ok(markers[0].message.includes(`Attribute 'applyTo' is not supported in rules files by BatikCode agents.`));
		});
	...
	suite('claude agents', () => {
		...
		test('Claude agent tools are not validated against BatikCode tool registry', async () => {
			// Claude tool names (Edit, Grep, etc.) don't exist in BatikCode's tool registry
			...
			const markers = await validate(content, PromptsType.agent, claudeAgentUri);
			assert.deepStrictEqual(markers, [], 'Claude tools should not be validated against BatikCode registry');
		});
		...
		test('Claude agent does not validate handoffs or agents attributes', async () => {
			// handoffs and agents are BatikCode-specific; they shouldn't be validated for Claude
			const content = [
				'---',
				'name: test-agent',
				'description: Test',
