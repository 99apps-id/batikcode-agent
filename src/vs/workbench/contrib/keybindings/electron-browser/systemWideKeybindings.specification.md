...
`keybindings.json` entries that fire even when BatikCode is not the focused application, backed by
...
While BatikCode is running (even unfocused), pressing that combination runs the command. Rules:
...
- **`when` clauses are ignored** for the global trigger — an OS-global shortcut has no editor/UI
  context, so it is always active while BatikCode runs. The user is warned once per binding label.
...
- `onTrigger(accelerator)` — resolves the target window: the focused window if it owns the
  ...
  routing window — a system-wide keybinding fires while BatikCode is typically unfocused, and pulling
