...
suite('PolicyExport Integration Tests', () => {
	...
	test('exported policy data matches checked-in file', async function () {
		...
		// This test launches BatikCode with --export-policy-data flag, so it takes longer
		this.timeout(60000);
		...
		try {
			// Launch BatikCode with --export-policy-data flag
			const scriptPath = isWindows
				? join(rootPath, 'scripts', 'code.bat')
				: join(rootPath, 'scripts', 'code.sh');

			// Skip prelaunch to avoid redownloading electron while the parent BatikCode is using it.
